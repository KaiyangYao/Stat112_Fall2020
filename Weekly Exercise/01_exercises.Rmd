...
![](Question4.jpg)
  ...
  [My favorite restaurant](https://www.ichiddo.com)
...
dim(penguins)
```
There are total 344 cases and 8 variables in the Palmer penguin data.
...
This data set contains the sex and size measurements for penguins near islands in Palmer Station, Antarctica. The size measurements include flipper length, body mass, and bill dimensions.
...
penguins %>% 
  ...
  geom_histogram(fill = "lightskyblue1", color = "grey67", bins = 40) +
  geom_vline(aes(xintercept = mean(bill_length_mm, na.rm = TRUE))) +
  labs(title = "Distribution of the bill length")
```

The bill length is distributed evenly with several peaks at around 40 and 50 mm. The mean of the bill length is around 44 mm. The span of the bill length is around 30 mm.
...
penguins %>% 
  ...
  geom_point() +
  labs(title = "Relationship between bill length and bill depth by species") +
  theme_minimal()
```

The bill length and bill depth are positively related with each other. In general, the correlation is high among all species through 3 similar clusters, and the correlation of Chinstrap and Gentoo seems a little bit higher than that of Adelie.
...
Male penguin has the highest IQR and the highest mean of bill length. Penguin with no sex has the lowest IQR and the lowest mean of bill length. Female penguin is in the middle. Generally, male penguin's bill length is larger.
...
penguins %>% 
  ...
  labs(title = "Bill length by sex of each species")
```

Despite the observation we made in the last question that males penguin's bill length is larger, we can see that the bill length of Chinstrap and Gentoo is overall greatly larger than that of Adelie.
...
## Introduction to Voicethread by introducing ourselves  
  ...
  16. Add a slide to this [Voicethread]https://macalester.voicethread.com/share/15918610/). Insert a video (preferred, but not required), photo with audio, or just audio introducing yourself to the class. State your name, pronouns, what excites you about the class, and what makes you nervous about the class. Keep the videos less than one minute. Mine is already up for you to watch. You will need to be logged into your Macalester account in order to add the slide. I also posted this under the Dear Data assignments on moodle. You may want to read that assignment description to learn how to add a slide to a Voicethread. You do not need to write anything here for this exercise.
