...
Problem: every time during the join, when there are multiple matches in the right table for a particular case in the left table, a separate case will be created for each of the matches from the right. So we get more rows than expected.
...
```
  ...
  From Monday through Friday, there are two distinct peaks each day, which is the time for on duty and off duty. In Saturday and Sunday, there is only one peak at the afternoon.
...
```
  ...
  Worse. When stacking the position, we are not able to tell the trend of each type of client separately.
...
```
  ...
  Results: Casual client are more likely to trip during the weekends, while registered client are more likely to trip during weekdays.
...
https://github.com/KaiyangYao/Stat112_Fall2020/blob/main/Weekly%20Exercise/03_exercises.Rmd
